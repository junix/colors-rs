...
def p_hero(insp):
    ...
    o.append(text(24, 346, f"同一种输入语言：十个语法族，命名色 {named_count} 个",
...
def p_spaces(chain, insp):
    ...
    left = [("HSL", insp["color"]["hsl"], "max/min 几何"),
            ("HSV", insp["color"]["hsv"], "max/min 几何"),
            ("CMYK", insp["color"]["cmyk"], "k = 1−max(R,G,B)")]
    ly = 40
    ...
    right = [
        ("线性 RGB", f"({lin['r']:.6f} {lin['g']:.6f} {lin['b']:.6f})",
         "v≤0.04045 ? v/12.92 : ((v+0.055)/1.055)^2.4"),
        ("OKLab", insp["color"]["oklab"],
         "LMS 矩阵 + 三次立方根"),
        ("OKLCH（极坐标）", insp["color"]["oklch"],
         "c=√(a²+b²)，h=atan2(b,a)"),
    ]
    ry = 40
...
def p_complement(comp, ratio, ensure):
    W, H = 1072, 548
    o = []
    # design-boundary statement card (verbatim crate-doc quote lives in
...
def p_luminance(chain, insp, selfchecks):
    ...
    cxx = 470
    steps = [
        ("① gamma sRGB", "(0.4  0.2  0.6)", "8 位整数 102 51 153 ÷ 255"),
        ("② linear sRGB",
...
def p_tokens(tok, png):
    ...
    o.append(srcnote(24, H - 14,
                     ...
                     "data/png-freeze.json（两次 --png 运行 sha256 相等）"))
...
def p_evidence(prov, engine, selfchecks, n_panels=11):
    ...
    nodes = [
        ...
        ("证据冻结", (f"{n_cmds} 组命令实跑", "浮点全精度冻结")),
        (f"data/ {n_files} 个 JSON", ("provenance.json", "记录每条完整 argv")),
        ("确定性生成", ("面板几何数据驱动", "输出无时间戳无路径")),
        ("断言渲染", ("y=0 全宽切片", "位图高 = CSS 高×dpr")),
    ]
    nx, nw = 24, 186
    x = nx
